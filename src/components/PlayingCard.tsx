...
export function PlayingCard({
  ...
  const rankText = size === 'xs' ? 'text-sm font-bold' : 'text-lg font-bold'
  const suitText = size === 'xs' ? 'text-xs' : 'text-base'
  const centerSymbol = size === 'xs' ? 'text-2xl' : 'text-4xl'
  const cardPad = size === 'xs' ? 'p-1' : 'p-2'
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={!clickable}
      className={`${sizeClass} card-deal rounded-xl bg-white shadow-xl flex flex-col items-start justify-between ${cardPad} relative overflow-hidden ${clickable ? 'cursor-pointer hover:scale-105 active:scale-95 transition-transform' : ''} ${selected ? 'ring-2 ring-gold ai-pulse' : ''}`}
      style={{ animationDelay: `${delay}ms` }}
    >
      <div className="flex flex-col leading-none" style={{ color }}>
        <span className={rankText}>{rankLabel}</span>
        <span className={suitText}>{symbol}</span>
      </div>
      <span className={`absolute inset-0 flex items-center justify-center ${centerSymbol} opacity-20`} style={{ color }}>
        {symbol}
      </span>
